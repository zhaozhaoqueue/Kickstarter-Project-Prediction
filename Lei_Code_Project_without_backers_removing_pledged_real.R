...
da_val = raw_val_data[, c(21, 9, 5, 3, 20, 14, 13, 11, 19, 18, 16, 15)]
...
# Tree  
# Tree predicts all the intances as "others".
library(tree)
# try with all the attributes
tree_fit = tree(state~., data=da_train, split="gini")
# plot unpruned tree
plot(tree_fit)
...
set.seed(0)
bag_fit = randomForest(state~., data=da_train, mtry=11, importance=TRUE, ntree=20)
bag_fit
...
summary(log_fit_best)
prob_log = predict(log_fit_best, newdata=da_oh_val, type="response")
pred_log = ifelse(prob_log>=0.7, "successful", "others")
table(pred_log, da_oh_val$state)
mean(pred_log == da_oh_val$state)
print("Accuracy of success")
431/(431 + 143)
...
# PCA
pca_train = prcomp(da_oh_train[, -3], center=TRUE, scale=TRUE)
summary(pca_train)
# plot PVE curve
par(mfrow=c(2, 1))
...
# plot data after PCA
par(mfrow=c(1, 1))
plot(pca_train$x[, c(1,3)], col=as.numeric(da_oh_train$state)+1)
...
# summary(v)
da_pca_val = data.frame(predict(pca_train, da_oh_val[, -3])[, 1:5], da_oh_val$state)
names(da_pca_val)[6] = "state"



da_pca_train_1 = data.frame(pca_train$x[, 1:24], da_oh_train$state)
names(da_pca_train_1)[25] = "state"
da_pca_val_1 = data.frame(predict(pca_train, da_oh_val[, -3])[, 1:24], da_oh_val$state)
names(da_pca_val_1)[25] = "state"


# Logistic regression after PCA
log_pca_fit = glm(state~., family=binomial, data=da_pca_train_1)
prob_log_1 = predict(log_pca_fit, newdata=da_pca_val_1, type="response")
pred_log_1 = ifelse(prob_log_1>=0.5, "successful", "others")
table(pred_log_1, da_pca_val_1$state)
...
library(MASS)
lda_fit_test = lda(state~., data=da_oh_train)
table(predict(lda_fit_test, da_oh_val)$class, da_oh_val$state)
...
da_oh_scaled_train = apply(da_oh_train[, -3], 2, my_scale)
da_oh_scaled_train = data.frame(da_oh_scaled_train, da_oh_train$state)
names(da_oh_scaled_train)[26] = "state"
da_oh_scaled_val = da_oh_val[, -3]
for(i in 1:ncol(da_oh_scaled_val)){
        train_data = da_oh_train[, -3]
...
names(da_oh_scaled_val)[26] = "state"
...
pred_test_knn = knn(da_oh_scaled_train[, -26], da_oh_scaled_val[, -26], da_oh_scaled_train$state, k=2)
1 - mean(pred_test_knn == da_oh_scaled_val$state)
table(pred_test_knn, da_oh_scaled_val$state)
print("Accuracy of success")
11708/(11708 + 11634)
...
for(j in 1:10){
        for(i in 1:10){
                testIndexes = which(folds==i, arr.ind=TRUE)
                testData = da_oh_scaled_train[testIndexes, -27]
                trainData = da_oh_scaled_train[-testIndexes, -27]
